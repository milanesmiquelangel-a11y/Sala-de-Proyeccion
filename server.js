...
function requireLogin(req, res, next) { if (!req.session.userId) return res.status(401).json({ error: 'Tienes que iniciar sesión.' }); next(); }
...
async function dbUserByEmail(email) {
  ...
  const { data, error } = await supabase.from('users').select('*').ilike('email', email).maybeSingle(); if (error) throw error; return data;
}
async function dbUserById(id) {
  if (!USE_SUPABASE) return readDB().users.find(u => u.id === id) || null;
  const { data, error } = await supabase.from('users').select('*').eq('id', id).maybeSingle(); if (error) throw error; return data;
}
async function createUser(user) {
  if (!USE_SUPABASE) { const db = readDB(); db.users.push(user); writeDB(db); return user; }
  const { data, error } = await supabase.from('users').insert(user).select().single(); if (error) throw error; return data;
}
async function listGenerations(userId) {
  ...
  const { data, error } = await supabase.from('generations').select('*').eq('user_id', userId).order('created_at', { ascending:false }).limit(50); if (error) throw error;
...
async function countRecentGenerations(userId) {
  ...
  const { count, error } = await supabase.from('generations').select('id',{count:'exact',head:true}).eq('user_id',userId).gt('created_at',since); if(error)throw error; return count||0;
}
async function createGeneration(g) {
  ...
  const { data,error }=await supabase.from('generations').insert(row).select().single(); if(error)throw error; return data;
}
async function getGenerationForUser(id,userId) {
  ...
  const {data,error}=await supabase.from('generations').select('*').eq('id',id).eq('user_id',userId).maybeSingle(); if(error)throw error;
...
async function deleteGeneration(id,userId) {
  ...
  const g=await getGenerationForUser(id,userId);if(!g)return null;const {error}=await supabase.from('generations').delete().eq('id',id).eq('user_id',userId);if(error)throw error;return g;
}
async function signedUrl(pathname,expires=3600){if(!pathname)return null;if(!USE_SUPABASE)return pathname;const {data,error}=await supabase.storage.from(STORAGE_BUCKET).createSignedUrl(pathname,expires);if(error)throw error;return data.signedUrl;}
async function storageUpload(filePath,buffer,contentType){
  ...
  const {error}=await supabase.storage.from(STORAGE_BUCKET).upload(filePath,buffer,{contentType,upsert:false,cacheControl:'31536000'});if(error)throw error;return filePath;
}
async function storageRemove(paths){const clean=paths.filter(Boolean);if(!clean.length)return;if(!USE_SUPABASE){for(const p of clean){const dir=p.startsWith('images/')?uploadsDir:videosDir;const f=path.join(dir,path.basename(p));if(fs.existsSync(f))fs.unlinkSync(f);}return;}const {error}=await supabase.storage.from(STORAGE_BUCKET).remove(clean);if(error)console.error('Storage remove:',error.message);}

app.post('/api/register',async(req,res)=>{try{const {email,password}=req.body||{};if(!email||!password||password.length<8)return res.status(400).json({error:'Correo y contraseña (mínimo 8 caracteres) son obligatorios.'});if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return res.status(400).json({error:'Introduce un correo válido.'});if(await dbUserByEmail(email))return res.status(409).json({error:'Ya existe una cuenta con ese correo.'});const user={id:crypto.randomUUID(),email:email.trim(),password_hash:await bcrypt.hash(password,10),created_at:new Date().toISOString()};await createUser(USE_SUPABASE?user:{id:user.id,email:user.email,passwordHash:user.password_hash,createdAt:user.created_at});req.session.userId=user.id;res.json({email:user.email});}catch(e){console.error('Register error:',e);res.status(500).json({error:'No se pudo crear la cuenta.'});}});
app.post('/api/login',async(req,res)=>{try{const {email,password}=req.body||{};const user=await dbUserByEmail(email||'');const hash=user?.password_hash||user?.passwordHash;if(!user||!(await bcrypt.compare(password||'',hash||'')))return res.status(401).json({error:'Correo o contraseña incorrectos.'});req.session.userId=user.id;res.json({email:user.email});}catch(e){console.error('Login error:',e);res.status(500).json({error:'No se pudo iniciar sesión.'});}});
...
async function generateWanVideo({imageUrl,prompt,duration}){
  ...
  if(/^https?:\/\//i.test(videoUrl)){const r=await fetch(videoUrl,{headers:{Authorization:`Bearer ${HF_TOKEN}`}});if(!r.ok)throw new Error(`No se pudo descargar el vídeo generado (${r.status}).`);return Buffer.from(await r.arrayBuffer());}
  throw new Error('Wan2.2 devolvió una ruta interna no descargable.');
}

app.post('/api/generate-video',requireLogin,async(req,res)=>{try{
  ...
  const id=crypto.randomUUID();const videoPath=`videos/${req.session.userId}/${id}.mp4`;await storageUpload(videoPath,buffer,'video/mp4');const createdAt=new Date().toISOString();
...
app.use((err,req,res,next)=>{console.error('Unhandled request error:',err);if(err instanceof multer.MulterError)return res.status(400).json({error:err.message||'Error en la carga del archivo.'});if(err instanceof SyntaxError&&'body' in err)return res.status(400).json({error:'JSON de la solicitud no válido.'});return res.status(500).json({error:'Error interno del servidor.'});});
